using AlohaKit.Layouts.Helpers;
...
namespace AlohaKit.Layouts
{
    ...
    /// <summary>
    /// The CircularLayout is a simple Panel derivative that lays out its children in a circular arrangement. 
    ...
    public class CircularLayout : Layout<View>
    {
        ...
        private void CreateLayout(double x, double y, double width, double height)
        {
            ...
            foreach (View element in Children)
            {
                ...
                Rect elementBounds = CreateCenteralizeRectangle(location, new Size(childSizeRequest.Request.Width, childSizeRequest.Request.Height));

                LayoutChildIntoBoundingRegion(element, elementBounds);
            }
        }

        private Rect CreateCenteralizeRectangle(Point point, Size size)
        {
